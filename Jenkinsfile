pipeline{
      ...
      stages{
        ...
        stage('Git'){
          steps{
            git branch: '${BRANCH}', credentialsId: '', url: 'https://github.com/aliuweizj/jenkins-demo.git'
          }
        }
        ...
        stage('Deploy to Kubernetes') {
            parallel {
                stage('Deploy to Production Environment') {
                    ...
                    steps {
                        container('kubectl') {
                            step([$class: 'KubernetesDeploy', authMethod: 'certs', apiServerUrl: 'https://kubernetes.default.svc.cluster.local:443', credentialsId:'k8sCertAuth', namespace:'ds-team-uat',config: 'deployment.yaml',variableState: 'ORIGIN_REPO,REPO,IMAGE_TAG'])
                        }
                    }
                }
